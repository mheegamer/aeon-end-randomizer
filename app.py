# ==============================================================================
# Aeon's End Randomizer - Final Version (Filter + Local Storage Save System)
# ==============================================================================
...
# --- จัดการ State ของแอป (ส่วนสำคัญที่เพิ่มเข้ามา) ---
if "expedition_data" not in st.session_state:
    st.session_state.expedition_data = None


# --- 6. ฟังก์ชันสำหรับแสดงผล (เพื่อลดโค้ดซ้ำซ้อน) ---
...
# (UI ส่วนฟิลเตอร์และเลือกโหมด เหมือนเดิม)
...
if st.sidebar.button("🎲 เริ่มสุ่มใหม่!"):
    ...
    try:
        if game_mode == "เล่นยาว (Expedition)":
            ...
            st.session_state.expedition_data = {
                "mode": "เล่นยาว (Expedition)",
                "initial_market": initial_market_df.to_dict('records'),
                "mages": mages,
                "nemeses": [nemesis_1, nemesis_2, nemesis_3, nemesis_4],
                "replacements": [replacement_market_1.to_dict('records'), replacement_market_2.to_dict('records'), replacement_market_3.to_dict('records')],
                "treasures": [treasures_1.to_dict('records'), treasures_2.to_dict('records'), treasures_3.to_dict('records')]
            }
        else: # Single Game
            st.session_state.expedition_data = {
                ...
                "mages": df_mages.sample(4).to_dict('records'),
                "market": pd.concat([
                    df_cards[df_cards['Type'] == 'Gem'].sample(3),
                    df_cards[df_cards['Type'] == 'Relic'].sample(2),
                    df_cards[df_cards['Type'] == 'Spell'].sample(4)
                ]).to_dict('records')
            }
        st.balloons()
        set("aeon_end_save", None) # ล้างเซฟเก่าเมื่อสุ่มใหม่
    except Exception as e:
...
if st.session_state.expedition_data:
    if st.session_state.expedition_data.get('mode') == 'เล่นยาว (Expedition)':
        display_expedition(st.session_state.expedition_data)
    else:
        display_single_game(st.session_state.expedition_data)
else:
